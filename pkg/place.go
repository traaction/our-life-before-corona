...
func (p Place) Add(c *gin.Context) {
	...
	c.JSON(http.StatusCreated, models.ReturnId{ID: place.ID})
}
...
func (p Place) List(c *gin.Context) {
	...
	query := "%" + c.Param("place") + "%"
	if err := p.DB.Table("places").Select("ID, Name").Limit(10).Where("Name ILIKE ?", query).Find(&places).Error; err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, err)
		return
	}
...
func (p Place) ListAll(c *gin.Context) {
	places := make([]models.ReturnNameId, 0)
	if err := p.DB.Table("places").Select("ID, Name").Find(&places).Error; err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, err)
		return
	}
	c.JSON(http.StatusOK, places)
